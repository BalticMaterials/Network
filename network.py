...
def mon_net_connection(ping_freq=2):
    ...
    motd = "Network connection monitoring started at: " + \
        str(monitor_start_time).split(".")[
            0] + " Sending ping request in " + str(ping_freq) + " seconds"
    print(motd)
    ...
    while True:
        if send_ping_request():
            ...
            print("Successfully ping done.")
        else:
            down_time = datetime.datetime.now()
            fail_msg = "Network Connection Unavailable at: " + \
            ...
            while not send_ping_request():
                ...
                if i >= 3600:
                    ...
                    continous_message = "Network Unavailabilty Persistent at: " + \
                        str(now).split(".")[0]
                    print(continous_message)
                    with open(FILE, "a") as file:
                        file.write(continous_message + "\n")
            up_time = datetime.datetime.now()
            uptime_message = "Network Connectivity Restored at: " + \
                str(up_time).split(".")[0]
